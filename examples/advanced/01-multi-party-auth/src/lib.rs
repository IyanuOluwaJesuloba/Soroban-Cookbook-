...
impl MultiPartyAuthContract {
    ...
    pub fn proposal_approval(
        env: Env,
        proposal_id: Symbol,
        approvers: Vec<Address>,
    ) {
        // Load the required threshold and the legitimate signatories list
        let required_threshold: u32 = env.storage().instance().get(&DataKey::Threshold(proposal_id.clone())).unwrap_or(2);
        let valid_signers: Vec<Address> = env.storage().instance().get(&DataKey::Signers(proposal_id)).unwrap_or_else(|| {
    ...
    pub fn sequential_auth_escrow(
        ...
        if step == 0 {
            ...
            env.storage().instance().set(&DataKey::EscrowBal(buyer.clone(), seller.clone()), &amount);
            ...
            env.storage().instance().set(&DataKey::EscrowBal(buyer, seller), &0i128);
        }
    }
    ...
    pub fn setup_proposal(
        ...
        env.storage().instance().set(&DataKey::Threshold(proposal_id.clone()), &threshold);
        env.storage().instance().set(&DataKey::Signers(proposal_id), &signers);
    }
}
